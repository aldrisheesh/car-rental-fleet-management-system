-- VS011 correction: bind closure to the booking reviewed by Owner/Admin.
...
revoke all on function public.return_vehicle_close_rental(uuid,uuid,uuid,uuid,timestamptz,numeric,text,text,text,text) from public,anon,authenticated;
grant execute on function public.return_vehicle_close_rental(uuid,uuid,uuid,uuid,timestamptz,numeric,text,text,text,text) to service_role;
