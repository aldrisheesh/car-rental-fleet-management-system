...
async function mutateBooking({ request }: { request: Request }) {
  try {
    ...
    if (action === "return" && (!text(body?.rentalId) || !text(body?.expectedVehicleId) || !text(body?.expectedStartedAt))) return errorResponse("Reload the active rental before returning.", 409);
