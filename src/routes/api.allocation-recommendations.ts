...
const authStatus = (error: unknown) =>
...
async function loadRecommendationView(client: any, batchId?: string) {
  let batchQuery = client
    ...
    .select("id,generated_by,generated_at,idempotency_key,created_at")
  ...
  const branchIds = [
    ...new Set(
      (recommendations.data ?? []).flatMap((r: any) => [
        r.source_branch_id,
  ...
  const categoryIds = [
    ...new Set(
      (recommendations.data ?? []).map((r: any) => r.vehicle_category_id),
    ),
  ];
  const evaluationIds = [
    ...new Set(
      (recommendations.data ?? []).flatMap((r: any) => [
        r.source_supply_evaluation_id,
        r.destination_supply_evaluation_id,
      ]),
    ),
  ];
  const [candidates, branches, categories, evaluations] = await Promise.all([
    recommendationIds.length
      ...
      : { data: [], error: null },
    branchIds.length
      ? client.from("branches").select("id,name").in("id", branchIds)
      : { data: [], error: null },
    categoryIds.length
      ? client
  ...
  const branchNames = new Map(
    (branches.data ?? []).map((x: any) => [x.id, x.name]),
  );
  const categoryNames = new Map(
    (categories.data ?? []).map((x: any) => [x.id, x.name]),
  );
  const evaluationTimes = new Map(
    (evaluations.data ?? []).map((x: any) => [x.id, x.evaluated_at]),
  );
  return {
    ...
    recommendations: (recommendations.data ?? []).map((r: any) => ({
      ...r,
      source_branch_name:
        branchNames.get(r.source_branch_id) ?? r.source_branch_id,
      destination_branch_name:
        branchNames.get(r.destination_branch_id) ?? r.destination_branch_id,
      vehicle_category_name:
        categoryNames.get(r.vehicle_category_id) ?? r.vehicle_category_id,
      source_evaluated_at:
        evaluationTimes.get(r.source_supply_evaluation_id) ?? null,
      destination_evaluated_at:
        evaluationTimes.get(r.destination_supply_evaluation_id) ?? null,
      candidates: (candidates.data ?? []).filter(
        (x: any) => x.recommendation_id === r.id,
      ),
    })),
  };
}
...
async function read() {
  try {
    ...
    return fail(
      error instanceof AuthBoundaryError
        ? error.reason === "forbidden"
          ? "Allocation recommendation access is restricted."
          : "Authentication required."
        : "Unable to load allocation recommendations.",
      authStatus(error),
    );
  }
}
...
async function generate({ request }: { request: Request }) {
  try {
    ...
    const contextFingerprint = `all-latest-v1:${latest
      .map((e) => e.id)
      .sort()
      .join(",")}`;
    const persisted = await client.rpc(
      "persist_allocation_recommendation_batch",
      {
    ...
    if (persisted.error) {
      const mismatch = String(persisted.error.message).includes(
        "idempotency_key_context_mismatch",
      );
      return fail(
        mismatch
          ? "Idempotency key was already used for a different generation context."
          : "Unable to persist the allocation recommendation batch atomically.",
        409,
      );
    }
    return Response.json(
    ...
    return fail(
      error instanceof AuthBoundaryError
        ? error.reason === "forbidden"
          ...
          : "Authentication required."
        : "Unable to generate allocation recommendations.",
      error instanceof AuthBoundaryError
        ? error.reason === "forbidden"
          ? 403
          : 401
...
async function decide({ request }: { request: Request }) {
  try {
    ...
    const current = await client
      .from("allocation_recommendations")
      .select("recommended_transfer_units")
    ...
    let decision;
    try {
      decision = validateAllocationDecision(
        body.state,
        body.approvedTransferUnits,
        Number(current.data.recommended_transfer_units),
      );
    } catch {
      return fail(
    ...
    if (result.error) {
      ...
      return fail(
        terminal
          ...
          : "Unable to record recommendation decision.",
        409,
      );
    }
    ...
    return fail(
      error instanceof AuthBoundaryError
        ? error.reason === "forbidden"
          ? "Owner/Admin access is required."
          : "Authentication required."
        : "Unable to decide allocation recommendation.",
      error instanceof AuthBoundaryError
        ? error.reason === "forbidden"
          ? 403
          : 401
