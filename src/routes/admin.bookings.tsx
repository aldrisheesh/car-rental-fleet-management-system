...
function BookingsPage() {
  ...
  useEffect(() => {
    if (!selected?.id || selected.booking_status !== "Submitted") { setOperationalContext(null); setContextError(""); setContextLoading(false); return; }
  ...
  }, [selected?.id, selected?.booking_status, vehicleId]);
  ...
  return (
    <div>
      ...
      {selected?.booking_status === "Submitted" && <OperationalContextPanel context={operationalContext} loading={contextLoading} error={contextError} advisoryNote="Advisory only. Existing assignment and confirmation checks remain authoritative." />}
