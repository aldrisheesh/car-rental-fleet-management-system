...
function MaintenancePage() {
  ...
  const rows = maintenance
    ...
    .sort((a, b) => {
      const prio =
        statusPriority[a.effectiveStatus] - statusPriority[b.effectiveStatus];
  ...
  const overdue = rows.filter((m) => m.effectiveStatus === "Open").length;
  const scheduled = rows.filter((m) => m.effectiveStatus === "Open").length;
  const inProgress = 0;
      ...
      <div className="mt-6 grid gap-4 2xl:grid-cols-[1.6fr_1fr]">
        ...
        <Card>
          <CardHeader
            ...
            hint="Days out of service per month"
          />
        ...
        onSave={() => {
          void fetch("/api/maintenance", {
            ...
            body: JSON.stringify({
              ...
              maintenanceType: draft.maintenance_type,
              description: draft.description,
              nextServiceDate: draft.scheduled_date,
              costPhp: draft.cost,
            }),
          });
        }}
