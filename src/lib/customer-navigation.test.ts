...
test("marks the booking list and per-booking routes as My Bookings paths", () => {
...
test("does not mark unrelated customer routes as My Bookings paths", () => {
  ...
  assert.equal(isMyBookingsPath("/customer/profile"), false);
  assert.equal(isMyBookingsPath("/customer/notifications"), false);
