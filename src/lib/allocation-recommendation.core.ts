...
export function generateAllocationDrafts(evaluations: AllocationEvaluation[], candidatesBySource: ReadonlyMap<string, AllocationCandidate[]>) {
  ...
  const dr = new Map(destinations.map((e) => [e.id, e.shortageUnits])); const sr = new Map(sources.map((e) => [e.id, e.surplusUnits])); const allocatedVehicleIds = new Set<string>(); const result: any[] = [];
  for (const destination of destinations) for (const source of sources) {
    ...
    const candidates = (candidatesBySource.get(source.id) ?? []).filter((candidate) => !allocatedVehicleIds.has(candidate.vehicleId));
    const units = Math.min(dr.get(destination.id) ?? 0, sr.get(source.id) ?? 0, candidates.length); if (units <= 0) continue;
    const selected = candidates.slice(0, units);
    result.push({ source, destination, recommendedUnits: units, candidates: selected }); dr.set(destination.id, (dr.get(destination.id) ?? 0) - units); sr.set(source.id, (sr.get(source.id) ?? 0) - units);
    for (const candidate of selected) allocatedVehicleIds.add(candidate.vehicleId);
  }
  return result;
}
