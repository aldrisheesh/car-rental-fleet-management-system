...
import { generateAllocationDrafts, rankAllocationCandidates, selectLatestEvaluations, validateAllocationDecision, type AllocationEvaluation } from "./allocation-recommendation.core.ts";
...
test("pairs only same category/week/horizon across branches", () => {
  ...
  assert.equal(generateAllocationDrafts([destination, source], new Map([["s", 2]])).at(0)?.recommendedUnits, 2);
  assert.equal(generateAllocationDrafts([destination, evaluation({ ...source, categoryId: "other" })], new Map([["s", 2]])).length, 0);
  assert.equal(generateAllocationDrafts([destination, evaluation({ ...source, targetWeekStart: "2026-09-14", targetWeekEnd: "2026-09-21" })], new Map([["s", 2]])).length, 0);
  assert.equal(generateAllocationDrafts([destination, evaluation({ ...source, branchId: "destination" })], new Map([["s", 2]])).length, 0);
});
...
test("remaining balances prevent source and destination oversubscription", () => {
  ...
  const rows = generateAllocationDrafts([d1, d2, s], new Map([["s", 3]]));
...
test("candidate count caps quantity and zero candidates skip pair", () => {
  ...
  assert.equal(generateAllocationDrafts([d, s], new Map([["s", 2]])).at(0)?.recommendedUnits, 2);
  assert.equal(generateAllocationDrafts([d, s], new Map([["s", 0]])).length, 0);
});
